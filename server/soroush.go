...
func recordSystemLog(message string, logType string) {
	// Skip writing low-level MTProto details to server database to avoid DB spam
...
func handleFetchGroups(w http.ResponseWriter, r *http.Request) {
	...
	// Find any available account with stored auth credentials
	...
	// Build and send getDialogs wrapped in initConnection (required per-connection)
